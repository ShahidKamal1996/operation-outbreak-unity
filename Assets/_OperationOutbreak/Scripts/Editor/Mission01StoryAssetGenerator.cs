...
namespace OperationOutbreak.EditorTools
{
    ...
    public static class Mission01StoryAssetGenerator
    {
        ...
        private static StorySequenceDefinition CreateOpening()
        {
            ...
            {
                // --- interior briefing (all camera world-jumps hidden by the fade controller) ---
                Beat(StoryBeatType.GameplayLock),
                CueBeat(StoryBeatType.EventCue, "m01_interior_setup", StoryCuePolicy.RequiredOnSkip),
                CueBeat(StoryBeatType.CameraCue, "m01_interior_kane", StoryCuePolicy.RequiredOnSkip),
                Dialog("raven_ortiz", "Corridor Seven went silent eleven minutes ago."),
                Dialog("sofia_reyes", "Evacuation teams stopped responding at Checkpoint Delta.", true),
                CueBeat(StoryBeatType.CameraCue, "m01_interior_kane_close"),
                Dialog("adrian_kane", "Any survivors?"),
                Dialog("raven_ortiz", "Unknown. Your job is to reach the checkpoint and find out."),
                WaitBeat(2f),
                CueBeat(StoryBeatType.CameraCue, "m01_interior_front"),
                Dialog("sofia_reyes", "Kane, once you are on the ground, you are on your own.", true),
                Dialog("raven_ortiz", "Thirty seconds. Gear up."),
                Dialog("adrian_kane", "Copy."),
                // --- interior -> exterior transition (faded) ---
                CueBeat(StoryBeatType.EventCue, "m01_interior_fade_out"),
                WaitBeat(0.7f),
                CueBeat(StoryBeatType.EventCue, "m01_interior_teardown", StoryCuePolicy.RequiredOnSkip),
                CueBeat(StoryBeatType.CameraCue, "m01_exterior_approach"),
                CueBeat(StoryBeatType.EventCue, "helicopter_approach"),
                CueBeat(StoryBeatType.EventCue, "m01_fade_in"),
                WaitBeat(2.5f),
                CueBeat(StoryBeatType.EventCue, "helicopter_insert", StoryCuePolicy.RequiredOnSkip),
                CueBeat(StoryBeatType.CameraCue, "m01_insertion"),
                CueBeat(StoryBeatType.EventCue, "m01_player_grounded", StoryCuePolicy.RequiredOnSkip),
