...
namespace OperationOutbreak.UI
{
 [DisallowMultipleComponent] public sealed class GameOverController : MonoBehaviour
 {
  ...
  void Awake(){ if(resultNavigation==null) resultNavigation=FindAnyObjectByType<MissionResultNavigation>(); Build(); }
  ...
  void Build(){
   ...
   var b=new GameObject("RetryButton",typeof(RectTransform),typeof(Image),typeof(Button));b.transform.SetParent(panel.transform,false);var br=(RectTransform)b.transform;br.anchorMin=br.anchorMax=new Vector2(.5f,.5f);br.sizeDelta=new Vector2(320,110);br.anchoredPosition=new Vector2(-180,-110);b.GetComponent<Image>().color=new Color(.2f,.75f,.32f,1);b.GetComponent<Button>().onClick.AddListener(RequestRetry);Text("RETRY",b.transform,Vector2.zero,38);
   var rb=new GameObject("ReturnButton",typeof(RectTransform),typeof(Image),typeof(Button));rb.transform.SetParent(panel.transform,false);var rbr=(RectTransform)rb.transform;rbr.anchorMin=rbr.anchorMax=new Vector2(.5f,.5f);rbr.sizeDelta=new Vector2(320,110);rbr.anchoredPosition=new Vector2(180,-110);rb.GetComponent<Image>().color=new Color(.2f,.75f,.32f,1);rb.GetComponent<Button>().onClick.AddListener(RequestReturn);Text("RETURN",rb.transform,Vector2.zero,38);
   panel.SetActive(false);
  }
  static void Text(string value,Transform parent,Vector2 pos,float size){var go=new GameObject(value,typeof(RectTransform),typeof(TextMeshProUGUI));go.transform.SetParent(parent,false);var r=(RectTransform)go.transform;r.anchorMin=r.anchorMax=new Vector2(.5f,.5f);r.sizeDelta=new Vector2(700,100);r.anchoredPosition=pos;var t=go.GetComponent<TextMeshProUGUI>();t.font=TMP_Settings.defaultFontAsset;t.text=value;t.fontSize=size;t.fontStyle=FontStyles.Bold;t.alignment=TextAlignmentOptions.Center;t.color=Color.white;t.raycastTarget=false;}
  ...
  void Show(){if(shown)return; if(missionComplete!=null&&missionComplete.IsVictory)return; shown=true;panel.SetActive(true);GameOverShown?.Invoke();} void Restart(){SceneManager.LoadScene(SceneManager.GetActiveScene().buildIndex);} void RequestRetry(){if(resultNavigation!=null)resultNavigation.RequestRetry();else Restart();} void RequestReturn(){if(resultNavigation!=null)resultNavigation.RequestReturn();else Debug.Log("[1V] Return requested - no Base/Map scene exists yet (2C+).",this);}
 }
}
