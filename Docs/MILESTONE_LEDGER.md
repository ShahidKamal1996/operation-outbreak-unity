...
| **1P.5** | **Toon Soldier visual integration** | **IMPLEMENTED — AWAITING MANUAL UNITY QA** | 2026-08-16 | commits on `arena/milestone-1p5-toon-soldier-integration` — presentation-only swap: soldier active, Carl inactive fallback, existing bridge reused, URP material applied. QA fix 1: controller rebuilt by editor tool from real clips (hand-authored FBX sub-asset fileIDs were invalid — "Clip Count: 0"); 3 regression tests. QA fix 2 (2026-08-17): FBX-derived grounding offset -1, presentation-only aim pivot (Player root never rotated), muzzle bound to the Right Hand bone with tunable barrel offset; 7 regression tests. QA fix 3 (2026-08-17): wrap test corrected to angular-equivalence assertions (runtime TurnToward was already correct; 181° == -179°), +1 boundary-matrix regression test. QA fix 4 (2026-08-17): muzzle now MEASURES the real rifle barrel tip (baked skinned mesh, forward-most vertex) and binds to a hand socket at that position; bounded bind retries; Unbind restores authored ownership for Carl fallback; +3 regression tests. QA fix 5 (2026-08-17): barrel-tip test corrected from exact Vector3 equality to a 1e-4 positional tolerance + selection guard (production was correct; float32 transform noise broke exact equality). QA fix 6 (2026-08-17): muzzle measured from the hand-rigid rifle cluster (pose-independent, immune to the bind-pose helmet/face that the retired global forward-most heuristic picked); +3 regression tests. QA fix 7 (2026-08-17): CS0165 compile fix - out variable moved out of the short-circuiting && expression with a semantically correct fallback initializer (runtime logic unchanged). QA fix 8/9 (2026-08-17): weapon presentation correction - prototype gun hidden while soldier active, muzzle FOLLOWS the hand socket instead of being re-parented (teardown parenting error eliminated), FBX-derived muzzle constants (0.543,-0.033,0.077 m hand-local; direction 0.989,-0.060,0.139); +4 regression tests. QA fix 10 (2026-08-17): CS0579 compile fix - one stray duplicate [Test] attribute removed (no test lost). QA fix 11 (2026-08-17): prototypeWeaponRoot scene reference corrected (Transform fileID 210011; was the GameObject 210010 -> null at runtime) and prototype-gun visibility decoupled from muzzle binding (hidden whenever the soldier visual is active); +2 renderer-state tests. QA fix 12 (2026-08-17): layered shooting - shoot animation moved off the Base Layer onto a dedicated upper-body masked layer (Empty default, exit-time blend-out) so firing no longer freezes locomotion legs; +4 animator-layer regression tests. QA fix 12A (2026-08-17): AvatarMask compile correction - transform APIs are index-based (3 x CS1503 fixed); Hips exclusion now resolves the mask index from GetTransformPath |
