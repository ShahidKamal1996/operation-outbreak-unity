...
| **1Q** | **Production enemy foundation (Basic Infected)** | **IMPLEMENTED — AWAITING MANUAL UNITY QA** | 2026-08-17 | commits on `arena/milestone-1q-production-enemy-foundation` — enemy presentation bridge, OO_BasicInfected.controller (idle/walk/attack/death; run reserved), StylizedZombie visual setup tool, death presentation window; +7 regression tests (144 total). QA fix 1 / Bug 4: walk cadence synced via a Walk-only LocomotionSpeedMultiplier parameter (reference derived from the clip's averageSpeed; gameplay 2.5 u/s unchanged; Attack/Death timing untouched); +3 tests (147 total). QA fix 1B / Bugs 1-3: asset-derived grounding offset (~-1.005 m), animation-safe restart-free hit flash with legacy punch gated to the prototype, clip-derived death window (length + 0.3 s); +4 tests (151 total) |
